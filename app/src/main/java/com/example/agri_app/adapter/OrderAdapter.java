...
public class OrderAdapter extends RecyclerView.Adapter<OrderAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        // 🌟 每次刷新先将按钮默认隐藏，防止复用导致的显示错乱
        holder.btnReceive.setVisibility(View.GONE);
        holder.btnReview.setVisibility(View.GONE);

        // 🌟 根据订单状态 (0:待发货, 1:已发货, 2:已收货/待评价, 3:已完成/已评价) 显示不同按钮
        if (o.getStatus() != null) {
            if (o.getStatus() == 0) {
                ...
                holder.btnReceive.setVisibility(View.VISIBLE); // 显示“确认收货”

            } else if (o.getStatus() == 2) {
                holder.tvStatus.setText("交易完成 (待评价)");
                holder.tvStatus.setTextColor(Color.parseColor("#4CAF50")); // 绿色
                holder.btnReview.setVisibility(View.VISIBLE); // 显示“评价订单”
            ...
            RetrofitClient.getApi().receiveOrder(currentOrder.getId()).enqueue(new Callback<Result<String>>() {
                ...
                public void onResponse(Call<Result<String>> call, Response<Result<String>> response) {
                    if (response.body() != null && response.body().code == 200) {
                        Toast.makeText(v.getContext(), "收货成功，请评价商品！", Toast.LENGTH_SHORT).show();
                        currentOrder.setStatus(2); // 状态变为 2 (待评价)
                        notifyItemChanged(currentPos); // 局部刷新
                    } else {
        ...
        holder.btnReview.setOnClickListener(v -> {
            ...
            // 跳转到我们刚才新做的 ReviewActivity
            Intent intent = new Intent(v.getContext(), ReviewActivity.class);
            intent.putExtra("ORDER_ID", currentOrder.getId());
            intent.putExtra("PRODUCT_ID", 1L); // 简化处理，默认评价商品1
    ...
    static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        Button btnReceive, btnReview; // 🌟 声明评价按钮
        ...
        public ViewHolder(View view) {
            ...
            btnReview = view.findViewById(R.id.btn_review); // 🌟 绑定评价按钮
        }
    }
}